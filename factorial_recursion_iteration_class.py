...
if __name__ == "__main__":
    n = int(input("\n정수를 입력하세요: ").strip())                  # strip : 문자열의 양끝 공백이나 개행 문자(\n, \t)를 제거. (xxDaxvidxx -> Daxvid)
    print(f"반복문 기반: {factorial_iter(n)}")
    try:                                                            # try~except : 프로그램 실행 중 오류 발생 시 예외를 처리하는 구문
        print(f"재귀 기반: {factorial_rec(n)}")
    except RecursionError:                                          # RecursionError : 재귀 함수 호출이 너무 깊어져서 한계를 초과했을 때 발생
