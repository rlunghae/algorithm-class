...
class LinkedList:
    ...
    def getNode(self, pos):
        ...
        # 리스트의 인덱스는 0부터 시작
        if pos < 0:
            return None                     # pos가 invalid한 경우
        if self.head == None:               # 주어진 리스트가 공백일 경우
            return None
        else:
            ptr = self.head
            for _ in range(pos):
    ...
    def insert(self, pos, elem):
        ...
        if pos < 0:
            return                          # 자동으로 유효하지 않는 위치에 대해 ValueError 발생
        
        # 1. 새 노드 생성
        new = Node(elem)                    # 또는 Node(elem, None)
        # 2. (pos - 1) 위치의 노드를 가져오기
